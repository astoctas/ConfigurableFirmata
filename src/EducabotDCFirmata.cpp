...
boolean EducabotDCFirmata::handlePinMode(byte pin, int mode)
{
  if (mode == OUTPUT) {
...
void EducabotDCFirmata::handleCapability(byte pin)
{
}
...
boolean EducabotDCFirmata::handleSysex(byte command, byte argc, byte *argv)
{
  if (command == L293D_DATA) {
    ...
    if (deviceNum < MAX_DC_OUTPUTS) {
      if (stepCommand == L293D_CONFIG) {
        ...
        analogWrite(outputs[deviceNum].pin_potencia, outputs[deviceNum].velocidad);
        ...
        analogWrite(outputs[deviceNum].pin_potencia, 0);
        outputs[deviceNum].running = 0;
      }
